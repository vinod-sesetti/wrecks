...
if dbteeth:
    ...
    from utils import create_or_update  #, setfields


#### main

if teeth:
    wget_command = "wget http://joe:%s@www.eracks.com:9080/eracks/admin/extract_order_lineitems.csv"
    wget_ask_command = "wget --ask-password http://joe@www.eracks.com:9080/eracks/admin/extract_order_lineitems.csv"
    ...
    fin = open ('extract_order_lineitems.csv')
    fout = open ('extracted_order_lineitems.csv', 'w')
    fout.write ('\n'.join ([l for l in fin.read().splitlines() [1:] if l]))
    fout.close()


reader = csv.DictReader(open('extracted_order_lineitems.csv', 'rb'), doublequote=False, escapechar='\\', skipinitialspace=True )  # , delimiter=' ', quotechar='|')
...
for row in reader:
    ...
    print 'importing', order_id, 'line item', line

    if dbteeth:
        def massage_date (dt):
            if dt:
                if dt == '4/2/02':  # 13373
                    dt = '2002-04-02 12:00:00'
                #if dt == '2002/11/12':
                #    dt = '2002-11-12 12:00:00'
                elif len (dt) < 12:
                    #print dt
        ...
        if not row ['shipdate']:
            ...
            row ['shipdate'] = massage_date (row ['shipdate'])

        row ['summary'] = unicode (row ['summary'], 'utf8', 'ignore')
        #row ['details'] = unicode (row ['details'].replace('\\x94','"').replace('0x94','"').replace('x94',''), 'utf8', 'ignore')
        #row ['details'] = row ['details'].encode ('string_escape', 'ignore')
        #row ['details'] = row ['details'].encode ('unicode_escape', 'ignore')
        #row ['details'] = row ['details'].encode ('raw_unicode_escape', 'ignore')
        #row ['details'] = row ['details'].encode ('unicode_internal', 'ignore')
        #row ['details'] = row ['details'].encode ('hex_codec', 'strict')
