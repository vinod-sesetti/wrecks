...
attrmap = dict (cls='class', class_='class', fr='for', d='id', id_='id')
...
def i     (c, *args, **kw):     return tag ('i', c, *args, **kw)
...
def thead (c, *args, **kw): return tag ('thead', c, *args, **kw)
def tbody (c, *args, **kw): return tag ('tbody', c, *args, **kw)
def tfoot (c, *args, **kw): return tag ('tfoot', c, *args, **kw)
...
def option (c, *args, **kw):
  ...
  if selected: return tag ('option', c, selected="selected", **kw)
...
def optionlist (*args, **kw):
  ...
  if trace: print islist (args), pformat (args), selected
...
def dropdown (*args, **kw):  # assumes you want a name in your dropdown :-)
  ...
  return select (optionlist (*args, **kw),
    name=kw.pop ('name', 'mydropdown'),
    cls=kw.pop ('cls', ''),
    )
