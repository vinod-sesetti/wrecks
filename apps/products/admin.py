...
class CCatForm (ModelForm):
    parent = ModelChoiceField (ChoiceCategory.objects.published(), widget=Select2())
...
#### Admin classes - Image - deprecating..
out='''
class ImageAdmin (admin.ModelAdmin):
    ...
    list_filter = ('updated', 'created','published')
    actions = [publish, unpublish]
    search_fields = ['image', 'title', 'caption']

class ImageInline (admin.TabularInline):
    model = Categories.images.through

    date_hierarchy = 'updated'
    fields = (image_thumbnail, 'image', updated, created,)  # 'image__title', 'image__caption', 'image__published'
    readonly_fields = (updated, created, image_thumbnail)
    actions = [publish, unpublish]
    search_fields = ['image', 'title', 'caption']
'''
