# JJW 010509 bloglet pluggable, from Greg Abbas, Joe Topian, and me, Joseph Wolff
# w/addl help from Marius Ursache March 11, 2008 at 1:55 a.m.
#
# ayp/xmlrpc.py
# Greg Abbas
# 2006 June 3
...
def view (req, thecall, module):
    dispatcher = SafeXMLRPCView(True, "utf-8")  # "iso-8859-1")  # fm Marius Ursache March 11, 2008 at 1:55 a.m.
    ...
    try:
        ...
        # get arguments
        if req.POST:
          data = req.raw_post_data
          if trace: print 'RAW_POST_DATA:', `req.raw_post_data`
        elif thecall:
          data=thecall # {thecall:1}
          if trace: print `req`
        else:
          data = dict (req.REQUEST)
          if trace: print `req`

        response = dispatcher._marshaled_dispatch (data, getattr(mod, '_dispatch', None))
    except:
        traceback.print_exc()
