...
#
# created 1999/02/02, Greg Ward
#

__revision__ = "$Id: dumper.py,v 1.11 1999/11/26 20:10:36 gward Exp $"
...
class Dumper:
    ...
    def __init__ (self, max_depth=None, instance_dump=None):
    ...
    def _dump (self, val, depth=0, indent=''):
        #print '_dump'
        t = type (val)
        ...
        if short_value (val):
            print indent + short_dump (val)

        else:
            depth = depth + 1

            if depth > self.max_depth:
                print "%s: suppressed (too deep)" % object_summary (val)
                return

            if self.seen.get (id (val)):
                print "%s: already seen" % object_summary (val)
                return

            self.seen[id(val)] = 1

            if t is DictionaryType:
                print object_summary (val)
                ...
                self.dump_instance (val, depth, indent)
    ...
    def dump_dict (self, dict, depth, indent):
        keys = dict.keys()
        keys.sort()
        ...
        for k in keys:
            val = dict[k]
            if short_value (val):
                print "%s%s: %s" % (indent, k, short_dump (val))
            else:
                print "%s%s:" % (indent, k),
                self._dump (val, depth, indent)
    ...
    def dump_sequence (self, seq, depth, indent):
        for i in range (len (seq)):
            val = seq[i]
            if short_value (val):
                print "%s%d: %s" % (indent, i, short_dump (val))
            else:
                print "%s%d:" % (indent, i),
                self._dump (val, depth, indent)


    def dump_instance (self, inst, depth, indent):
        print object_summary (inst),
...
def short_value (val):
    ...
    if type (val) not in (ListType, TupleType, DictionaryType, InstanceType):
        return 1

    elif type (val) in (ListType, TupleType) and len (val) <= 10:
        for x in val:
            if not atomic_type (type (x)):
                return 0
        return 1

    elif type (val) is DictionaryType and len (val) <= 5:
...
def short_dump (val):
    if  atomic_type (type (val)) or type (val) is ClassType:
        return `val`
...
def object_summary (val):
    ...
    if t is InstanceType:
        return "<%s instance at 0x%x>" % (val.__class__.__name__, id (val))

    elif t is ClassType:
        return "<%s %s at 0x%x>" % (TYPE_NAMES[t], val.__name__, id (val))

    else:
        try: 
            return "<%s at 0x%x>" % (TYPE_NAMES[t], id (val))
        except:
            return repr (t) # + ': ' + repr (val)
...
def zdump (self):
    import StringIO
    sys.stderr=sys.stdout=fd=StringIO.StringIO()
    try:
      try:
        print 'dir (self):', dir (self)

        try: print 'vars (self):', vars (self)
        except: print 'exception'
        
        if hasattr (self, '__name__'):
          name = self.__name__ + ' (' + repr (self) + ')'
        else:
          name = repr (self)
        
        if hasattr (self, '__doc__'):
          print name, '__doc__:', self.__doc__
        
        if hasattr (self, '__methods__'):
          print name, '__methods__:', self.__methods__
        
        if hasattr (self, '__members__'):
          print name, '__members__:', self.__members__

        if hasattr (self, '__d'):
          print name, '__d:', self.__d

        if hasattr (self, 'namespace'):
          print name, 'namespace.__name__:', self.namespace.__name__
          print name, 'namespace.__doc__:', self.namespace.__doc__
          print name, 'func_globals:', self.namespace.func_globals
          print name, 'func_name:', self.namespace.func_name
          #print name, '__name__:', self.__name__, 
          print name, 'func_code:', self.namespace.func_code
          print name, 'func_defaults:', self.namespace.func_defaults
          print name, 'func_doc:', self.namespace.func_doc
        ...
        #  print name, '__dict__:', self.__dict__

        
        print name + ' recursive dump:'        
        default_dumper.dump (self)
        
        if hasattr (self, '__dict__'):
          print name, '__dict__:'
          default_dumper.dump (self.__dict__)
        
        #if hasattr (self, '__d'):
        #  print name, '__d:'
        #  default_dumper.dump (self.__d)
        
        if hasattr (self, 'aq_chain'):
          print name, 'aq_chain:', `self.aq_chain`
          default_dumper.dump (self.aq_chain)
        
        #print 'done'
        
        if hasattr(self, '__class__'):
          print name, '__class__:', `self.__class__`
          default_dumper.dump (self.__class__)

        if hasattr (self, '_pop'):
          self = self._pop()
          while (self):
            print name, 'self._pop():', `self`
            #if hasattr (self, 'namespace'):
             #pass
             #print name, 'namespace.__name__:', self.namespace.__name__
             #print name, 'namespace.__doc__:', self.namespace.__doc__
             ...
             #print name, 'func_defaults:', self.namespace.func_defaults
             #print name, 'func_doc:', self.namespace.func_doc
            #zdump (self) nope, writes into new stringio obj, returns it, which gets thrown away...
            #print zdump (self) nope, doesn'twork either.
            default_dumper.dump (self)
            self = self._pop()
    
      except:
        print "zdump Exception:", sys.exc_info()[0]
    finally:
      printed = fd.getvalue()
      fd.close()
      return printed
