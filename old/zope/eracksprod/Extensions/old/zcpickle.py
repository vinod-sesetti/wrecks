...
def addOrder2 (self):	# assumes called fm localFS .pkl item
  ...
  try:
    ...
    result += "(Couldn't add folder %s - Already present?)" % order
...
def addOrders (self, dummy):	# assumes called fm localFS item itself
  result = 'dummy: %s req: %s \n' % (dummy, self.REQUEST)
