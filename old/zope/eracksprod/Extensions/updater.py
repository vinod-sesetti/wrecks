# up getpoc to use SERVE as default server (was NAS) 12/10/07 JJW
...
def getpoc (self, sku):
  ...
  for opt in opts.values():
      # here: jjw 2/11/12
      # if new_grid: loop thru opt.qty, set lineid - see fill_session
...
def sesurl (ses, req, url, host=0):                     # called fm here
  ...
  if req.environ.has_key('HTTP_COOKIE'): # JJW 8/16/3 - https name now matches http name, so no need for this: and host != 2:
    ...
    if '?' in url:
      # JJW 8/28/10 add &amp; for w3c validator
