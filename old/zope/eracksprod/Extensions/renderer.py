...
def li (s, cls=None):  # JJW 10/18/07 for styles
  if cls:
    return '<li class="%s">%s</li>' % (cls, s)
  else:
    return '<li>' + s + '</li>'
...
def comparepricesandalpha (c1, c2):
  try:
    p1 = int (c1.get ('pricedelta', 0))
  except:
    p1 = 0

  try:
    p2 = int (c2.get ('pricedelta', 0))
  except:
    p2 = 0
...
def comparesortorderandprices (c1, c2):
  ...
  if so1 != so2:
    ...
    return int (c1.get ('pricedelta', 0)) - int (c2.get ('pricedelta', 0))
...
def choicecombo (self, opt):  # assumes called form configure, w/valid session - also computes total price
  ...
  for choice in choicelist:
    ...
    #try:
    #  price = int (price)  # if not str(price).isdigit():
    #except Exception, e:
    #  raise `e` + '\nnonnint price: choiceid %s price %s' % (choiceid,price)
...
def cartbox2 (self, clr='#c4c4ff'):  # JJW 10/18/07
  def surl (url, flag=0, self=self):                   # self.session.url
    return updater.sesurl (self.session, self.REQUEST, url, flag)
  ...
  if totqty:     # extra td/td is for colspan=4 in boxtop/bottom - still renders wrong in netscape 4.6
    s = '<div class=box><div class=boxtitle>Your eRacks cart</div><div class="boxbody center">' + \
          link (surl ('cart', 1), img (self, 'cart') + \
                ' %i item%s' % (totqty, iff (totqty==1, '', 's')) + \
                ' $%8.2f total' % grandtot) + '<br>\n' + \
          link (surl ('cart', 1), 'View Cart') + '&nbsp&middot&nbsp' + \
          link (surl ('checkout', 2), 'Checkout') + '</div></div>\n'
  else:
    s = ''
...
def imgstr (im, *folders):
  ...
  try:
    ...
    return (' src="/%s" height="%s" width="%s" alt="%s" border="%s">' % \
           ...
           # JJW 5/24/11 change to relative path so SSL works - did this ever work?
           (img.virtual_url_path(), img.height, img.width, img.title,
...
def img (self, im):
  (s, theimg) = imgstr (im, self.buttons, self.images, self.images.logos)
  return '<img' + s
