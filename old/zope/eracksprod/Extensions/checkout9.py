# JJW 8/17/3 fix to call sendorder2
# JJW 8/17/3 upd eracks.net to eracks.com
# JJW 8/17/3 fix lopped off digit(s) in text email, add USD
# JJW 8/17/3 fix tax # incorrectly diplayed in text email 
# JJW 8/18/3 add CVV code - IN PROGRESS 2p
# JJW 8/20 CVV done
# JJW 8/21 sep shipping pmt options done, referral typ/src done, 
# JJW 8/22/3 make spec instr back to "2" lines

# JJW todo 8/21: Notes field bug
# JJW todo 8/21: complete descrip, not just 'Default configuration'

#__debug__ = 1

import myconfig
import myutil, tags, catax, statecodes, countrycodes, ccards, renderer, sendorder2
from myutil import *
from tags import *
...
ships =    ( ( "('Ground',1.0)", 'Ground'),  # note about insured - disclaimer for insurance claim process!
             ( "('3day',1.8)",   '3-day'),
             ( "('2day',2.4)",   '2-day'),
             ( "('1day',3.5)",   'Next Day'), 
...
ship     | shipmeth |         |   | 1 | ships | Shipping method  | Standard/expedited/international shipping  | 
choice   | shipper  |         |   | 1 | shippers | Preferred Shipper | Customer's preferred shipper           | 
rgroup   | shippay  |         |   |   |       | Ship pay method  | Shipping payment method - est/advance/included, cust ccard, cust acct  |
 radio   | shipincl | shippay |   |   |       | Estimated        | Estimated shipping included with order | Shipping info 
 radio   | shipcard | shippay | 1 |   |       | Actual           | Actual shipping charged separately (credit card orders only) | 
 radio   | shipcust | shippay |   |   |       | Customer acct    | Bill customer account number: %s using above shipper | 
  text   | shipacct | shipcust|   |   |       | Customer shipper #  | Customer shipper account number                    |                
tax      | tax      |         |   |   | taxes | Sales tax (CA only) | California shipping recipients please choose county | 
textarea | instr    |         |   |   |       | Special Instructions| Any special notes or instructions regarding your order | 
choice   | reftyp   |         |   | 1 | heards| Referral type     | Where did you hear of us?           | More referral Info
 text    | refsrc   | reftyp  |   |   |       | Referral Source   | Additional referral information     | 
...
paydefs = '''\
form     | payinfo  |         |   |   |       | Payment Info     | Complete your payment info: &nbsp;&nbsp; <img height=20 src="images/ccards4.gif"> |
rgroup   | paymeth  |         |   |   |       | Payment Method   | Choose your payment method     |
# radio   | byccard  | paymeth | 1 |   |       | Credit Card      | We accept: Visa/MC/Amex/Discover <br> Number %s Expiry %s %s CVV Code %s (on back) | 
 radio   | byccard  | paymeth | 1 |   |       | Credit Card      | Number %s Expiry %s %s CVV Code %s (on back) | 
 radio   | bypo     | paymeth |   |   |       | Purchase Order   | Company purchase order (PO) requires PO number, above | requires credit approval
 radio   | bycheck  | paymeth |   |   |       | Check            | Pay by company check (US funds only - will delay shipping)      | More info 
 radio   | bywire   | paymeth |   |   |       | Wire Transfer    | Pay by wire transfer (US or Int'l - will require instructions)  | More info
  ccard  | ccnum    | byccard |   |   |       | Number           | <img src="images/ccards4.gif"> |                
  inchoice| ccmonth | byccard |   |   | ccmonths | Expiry Month  | Expiration date month          |                
  inchoice| ccyear  | byccard |   |   | ccyears  | Expiry Year   | Expiration date year           |                
  text8    | cc_cvv | byccard |   |   |          | CVV Code      | CVV Code (on back of card)     | 
# JJW 2/8/07 - fixed - test! JJW 2/19/06 gives "unpack list of wrong size" error 
checkbox | iagree   |         |   | 1 |       | Agree to Terms | By checking here, I agree to eRacks' terms & conditions of sale, <a href="policies/orderinfo.html">listed here</a> | <a href="policies/orderinfo.html">Terms & Conditions</a> 
...
class text8 (field):  # roll this duplication into text at some point!  self.size = 8, for example 
  def renderin (self):
    if self.value:
      s = input (name=self.name, value=self.value, size=8)  # default type is 'text', so don't use it
    else:
      s = input (name=self.name)
...
class textarea (field):
  def renderin (self):
    s = font2 (tags.textarea (self.value, rows=2, cols=40, name=self.name))  # '<font face="arial,helvetica,sans-serif" size="2"><textarea rows="2" cols="60" name="notes"></textarea></font>'
...
class choice (field):
  def renderin (self):
    s = select (option (self.data, selected=self.value), name=self.name, style="max-width:250px")
...
class ship (choice):    
  ...
  def check (self, req):
    ...
    if req.shippay == 'shipcard' and req.paymeth != 'byccard':
      self.error = '"Actual charges" shipping payment requires credit card order'

    if req.shippay == 'shipcust' and not req.shipacct:
      self.error = '"Customer acct" shipping payment method requires shipper account number' 
...
class page:
  def invoicegrid (self, cart, ordfrm):   # all the '...grid' funcs also render a text version for email - should be more O-O!
    ...
    meth, shipping = eval (ord.shipmeth.value)   # 'shipping' here is the multiplier!
    carrier        = ord.shipper.value
    acct           = ord.shipacct.value
    shippay	   = ord.shippay.value

    shipping = max (5, lbs * shipping)   # now it's $$$, not mplier!
    taxdollars = tot * (float (tax) / 100)

    grandtot = tot + taxdollars # + shipping

    if not carrier in ['Any','None'] and not meth == 'Willcall':
      meth = meth + ', ' + carrier
       
    shiplabel= 'Shipping & Handling (%s)' % meth
    totlabel = 'Grand Total'

    renderedshipping = '$ %8.2f' % shipping
    
    if shippay == 'shipincl':
      grandtot += shipping
    else:  
      shiplabel = 'Shipping (%s, estimated $ %8.2f)' % (meth, shipping)
      renderedshipping = '$ 0.00'
            
    if shippay == 'shipcard':
      renderedshipping = '(billed directly to ccard)'
    
    if shippay == 'shipcust':
      shiplabel += ' (chged to %s acct #%s)' % (carrier, acct)      
    ...
    if tax:
      ...
      text = text + '%s: USD $ %8.2f \n' % (taxlabel, taxdollars)

    grid = grid + tr (tdf (shiplabel, colspan=5, align='right') + tdf (renderedshipping, align='right'))
    grid = grid + tr (thf (totlabel,  colspan=5, align='right') + thf ('USD $ %8.2f' % grandtot, align='right'))
    text = text + '%s: USD %s \n' % (shiplabel, renderedshipping)
    text = text + '%s: USD $ %8.2f \n' % (totlabel, grandtot)
  ...
  def saveorder (self, ses, cust, payfrm, orderno):
    zopeorders = os.path.join (myconfig.orderpath, str (orderno))
    txt = self.ordertext
    htm = self.orderhtml
    us  = 'orders@eracks.com'
    #us  = 'info@eracks.com'
    ...
    ccard = ''     
    if pay.paymeth.value == 'byccard':
      ccard = 'ccard: %s %s %s/%s' % (pay.ccnum.value, pay.cc_cvv.value, pay.ccmonth.value, pay.ccyear.value)
    ...
    url = myconfig.orderhost + '/%s.html' % orderno

    # JJW 8/17/3 use sendorder2, build subj into msg
    msg = 'Subject: Order #%s received\n\nOrder number is #%s:\n\n%s\n' % (orderno, orderno, url)
    sendorder2.asyncsendorder (us, us, msg)
    ...
    # JJW 8/17/3 use sendorder2, build subj into msg
    msg = 'Subject: Your order #%s\n\nThank you for your order. Your order number is #%s.\n It is now being processed.\n%s' % (orderno, orderno, txt)
    sendorder2.asyncsendorder (cust, us, msg)
  ...
  def handler (self, zope):
    ...
    if step == 1:    
      ...
      url = myconfig.securehost + '/checkout?session=%s' % ses.getName()
