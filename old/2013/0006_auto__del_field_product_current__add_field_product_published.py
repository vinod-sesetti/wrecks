...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column(u'products', 'published', self.gf('django.db.models.fields.BooleanField')(default=True), keep_default=False)

        # transferring current to published
        for prod in Product.objects.all():
            print prod, 'current:', prod.current
            prod.published = prod.current in ['T','t']
            prod.save()
            print prod, 'published:', prod.published

        # Deleting field 'Product.current'
        db.delete_column(u'products', 'current')


    def backwards(self, orm):
        from apps.legacy.models import Product

        # Adding field 'Product.current'
        db.add_column(u'products', 'current', self.gf('django.db.models.fields.CharField')(default='T', max_length=1), keep_default=False)

        # transferring published to current 
        for prod in Product.objects.all():
            print prod, 'published:', prod.published
            prod.current = 'T' if prod.published else 'F'
            prod.save()
            print prod, 'current:', prod.current

        # Deleting field 'Product.published'
        db.delete_column(u'products', 'published')
