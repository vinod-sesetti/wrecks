...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        from apps.legacy.models import Addresses, ChoiceCategory

        db.rename_column(u'addresses', 'dt', 'updated')

        # Adding field 'Addresses.created'
        db.add_column(u'addresses', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 7, 8), blank=True), keep_default=False)

        for address in Addresses.objects.all():
            address.created = address.updated
            address.save()

        # Adding field 'ChoiceCategory.created'
        db.add_column(u'choicecategories', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 7, 8), blank=True), keep_default=False)

        for cc in ChoiceCategory.objects.all():
            cc.created = cc.updated
            cc.save()


    def backwards(self, orm):

        db.rename_column(u'addresses', 'updated', 'dt')

        # Deleting field 'Addresses.created'
        db.delete_column(u'addresses', 'created')

        # Deleting field 'ChoiceCategory.created'
        db.delete_column(u'choicecategories', 'created')
