...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        from apps.legacy.models import Option, Categories

        db.rename_column(u'categories', 'dt', 'updated')

        # Adding field 'Categories.created'
        db.add_column(u'categories', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 7, 8), blank=True), keep_default=False)

        for cat in Categories.objects.all():
            cat.created = cat.updated
            cat.save()



        db.rename_column(u'options', 'dt', 'updated')

        # Adding field 'Option.created'
        db.add_column(u'options', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 7, 8), blank=True), keep_default=False)

        for opt in Option.objects.all():
            opt.created = opt.updated
            opt.save()


    def backwards(self, orm):

        db.rename_column(u'categories', 'updated', 'dt')

        # Deleting field 'Categories.created'
        db.delete_column(u'categories', 'created')


        db.rename_column(u'options', 'updated', 'dt')

        # Deleting field 'Option.created'
        db.delete_column(u'options', 'created')
