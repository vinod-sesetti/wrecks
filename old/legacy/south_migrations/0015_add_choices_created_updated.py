...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.add_column(u'choices', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 7, 9), blank=True), keep_default=False)
        db.add_column(u'choices', 'updated', self.gf('django.db.models.fields.DateTimeField')(auto_now=True, default=datetime.date(2011, 7, 9), blank=True), keep_default=False)

        from apps.legacy.models import Choice

        for c in Choice.objects.all():
            # THIS DOESN'T WORK.  note sure why. created & updated are today.
            # fixed up with choices_fixdates.sql
            c.created = c.dt  
            c.updated = c.dt
            c.save()
    ...
    def backwards(self, orm):
        db.delete_column(u'choices', 'created')
        db.delete_column(u'choices', 'updated')
